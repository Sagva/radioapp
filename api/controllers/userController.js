...
const editUserName = (req, res) => {
    ...
    db.run(query, params, function (err) { //first change name
        if (err) {
            res.status(401).json({ error: "Some error happened" })
        }
        let query = `SELECT * FROM users WHERE id = $id`
        let params = { $id: req.body.id }

        db.get(query, params, (err, userInDB) => { //after name was changed, get user from DB and set req.session.user, for renewing info about the current user
            if (!userInDB) {
                res.status(401).json({ error: "Some error happened" })
                return
            } else {
